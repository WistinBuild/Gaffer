...
// "We're live on Solana devnet" announcement — Gaffer dark/gold lockup over a
// Solana-themed field (purple #9945FF → green #14F195 gradient, light beams,
// confetti). 1600x900 (16:9) for an X in-stream post.
const APP = "/workspaces/Gaffer/packages/app/public";
const OUT = path.join(APP, "solana-live.jpg");
const W = 1600, H = 900;
const CX = W / 2;
...
// ── deterministic confetti (stable output) ───────────────────────────────────
const COLORS = ["#9945FF", "#14F195", "#E7C75A", "#FFFFFF", "#C68BFF"];
let confetti = "";
for (let i = 0; i < 64; i++) {
  const x = (i * 197.3) % W;
...
// ── radiating light beams behind the lockup (Solana gradient) ─────────────────
let beams = "";
const BEAM_CY = 388;
for (let i = 0; i < 12; i++) {
...
<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="bg" x1="0" y1="0" x2="0.6" y2="1">
      ...
      <stop offset="42%" stop-color="#120A24"/>
      <stop offset="100%" stop-color="#05070B"/>
    </linearGradient>
    <radialGradient id="purpleGlow" cx="50%" cy="40%" r="62%">
      <stop offset="0%" stop-color="#9945FF" stop-opacity="0.50"/>
      <stop offset="60%" stop-color="#1A0B33" stop-opacity="0"/>
    </radialGradient>
    <radialGradient id="greenGlow" cx="50%" cy="64%" r="46%">
      <stop offset="0%" stop-color="#14F195" stop-opacity="0.28"/>
      <stop offset="100%" stop-color="#14F195" stop-opacity="0"/>
    </radialGradient>
    <radialGradient id="goldGlow" cx="50%" cy="43%" r="32%">
      <stop offset="0%" stop-color="#E7C75A" stop-opacity="0.42"/>
    ...
    <linearGradient id="solText" x1="0" y1="0" x2="1" y2="0">
  ...
  <g opacity="0.5" stroke="#A06CFF" stroke-opacity="0.16" fill="none" stroke-width="2">
  ...
  <!-- live pill -->
  <g filter="url(#soft)">
    <rect x="${CX - 250}" y="120" width="500" height="58" rx="29" fill="#05070B" fill-opacity="0.6" stroke="#9945FF" stroke-opacity="0.75"/>
    <circle cx="${CX - 213}" cy="149" r="8" fill="#14F195"/>
    <text x="${CX - 183}" y="158" text-anchor="start" font-family="DejaVu Sans Mono, monospace" font-weight="bold"
          font-size="22" letter-spacing="4.5" fill="#C9B6FF">LIVE ON SOLANA DEVNET</text>
  </g>
  ...
  <text x="${CX}" y="552" text-anchor="middle" filter="url(#soft)"
        ...
        font-size="64" letter-spacing="2" fill="#FFFFFF">NOW PLAYING ON <tspan fill="url(#solText)">SOLANA</tspan></text>

  <!-- detail row -->
  <g filter="url(#soft)">
    <rect x="${CX - 360}" y="600" width="720" height="58" rx="29" fill="#05070B" fill-opacity="0.55" stroke="#14F195" stroke-opacity="0.45"/>
    <text x="${CX}" y="637" text-anchor="middle"
          font-family="DejaVu Sans Mono, monospace" font-weight="bold"
          font-size="22" letter-spacing="2" fill="#FFFFFF" fill-opacity="0.92">4 ANCHOR PROGRAMS · <tspan fill="#14F195">DEVNET</tspan> · PAY IN <tspan fill="#E7C75A">USDC</tspan></text>
  </g>
